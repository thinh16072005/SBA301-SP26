...
import { getAllOrchids } from '../services/orchidServices';
...
function OrchidList({ searchQuery = '', onHomeClick }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            await getAllOrchids().then(data => setOrchids(data))
        };
        fetchData();
    }, []);
    ...
    const filteredOrchids = useMemo(() => {
        if (!orchids) return [];
        let filtered = orchids.filter(orchid =>
        ...
        return filtered;
    }, [filterCategory, searchQuery, sortOption, orchids]);

    // Lấy danh sách các danh mục duy nhất từ mảng OrchidsData để truyền vào component FilterSort
    // Sử dụng useMemo để chỉ tính toán lại khi orchids thay đổi
